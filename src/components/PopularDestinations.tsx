          ...
          <Link href={`/destinations/${dest.id}`} key={dest.id} className="min-w-[85vw] md:min-w-0 snap-center group block h-full">
            <Card className="glass-premium overflow-hidden h-full flex flex-col cursor-pointer hover-lift-premium">
              <div className="relative h-64 overflow-hidden shrink-0">
                ...
                />
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full flex items-center gap-1.5 text-sm font-bold shadow-sm border border-green-100">
                  <Star className="w-4 h-4 fill-secondary text-secondary" />
                  <span className="text-gray-900">{dest.rating}</span>
                </div>
              </div>
              <CardContent className="p-5 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xl font-bold font-['var(--font-playfair)'] text-gray-900">{dest.name}</h3>
                  <span className="text-xs font-semibold text-primary bg-orange-50 border border-orange-100 px-2 py-1 rounded-md whitespace-nowrap ml-2">
                ...
                <div className="flex flex-wrap gap-2 mt-auto">
                  {dest.interests.slice(0, 3).map((interest, i) => (
                    <span key={i} className="text-xs bg-orange-50 text-orange-700 px-3 py-1 rounded-full font-medium border border-orange-100">
                      {interest}
                    </span>
                  ))}
                  {dest.interests.length > 3 && (
                    <span className="text-xs bg-gray-50 text-gray-600 px-3 py-1 rounded-full font-medium border border-gray-200">
                      +{dest.interests.length - 3}
                    </span>
                  )}
                </div>
